...
def upgrade() -> None:
    ...
    op.create_table("pe_cashflow",
        sa.Column("cf_id", sa.BigInteger, primary_key=True),
    ...
    op.create_table("pe_nav_observation",
        sa.Column("nav_obs_id", sa.BigInteger, primary_key=True),
        sa.Column("fund_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("investor_id", postgresql.UUID(as_uuid=True)),
        ...
        sa.Column("restates_nav_obs_id", sa.BigInteger),
        sa.Column("doc_id", postgresql.UUID(as_uuid=True)),
        sa.Column("source_trace", sa.JSON),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
    )
    ...
    op.create_table("ingestion_job",
        ...
        sa.Column("pipeline", sa.ARRAY(sa.Text)),
        sa.Column("started_at", sa.DateTime(timezone=True)),
        sa.Column("finished_at", sa.DateTime(timezone=True)),
        sa.Column("error_message", sa.Text),
        sa.Column("logs", sa.JSON, server_default=sa.text("'[]'::json")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
    )
